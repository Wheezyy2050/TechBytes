...
export default function CreatePost() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-8 text-[var(--text-primary)]">New Post</h1>
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        <div>
          <label className="text-xs uppercase tracking-wider text-[var(--text-muted)] block mb-2">Title</label>
          <input type="text" required className="w-full bg-[var(--bg)] border border-[var(--border)] p-3 text-sm focus:outline-none focus:border-[var(--accent)] transition-colors text-[var(--text-primary)]" value={form.title} onChange={handleTitleChange} />
        </div>
        <div>
